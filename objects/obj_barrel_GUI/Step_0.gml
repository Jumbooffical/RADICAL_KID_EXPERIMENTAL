...
if (obj_player.open_inventory) {
	...
	&& is_dragging) {
		if position_meeting(device_mouse_x_to_gui(0), device_mouse_y_to_gui(0), obj_inventory) {
			obj_inventory.inv.add(name, description, mouse_index, 4, type.ATTACHMENT, object)
			with(obj_player) {
				quickslot[selected_item, QSlot.Barrel] = 0
			}
		}
		mouse_index = 0;
		is_dragging = false;
	...
	&& (position_meeting(device_mouse_x_to_gui(0), device_mouse_y_to_gui(0), id)) {
		obj_inventory.inv.add(name, description, sprite_index, 4, type.ATTACHMENT, object)
		with(obj_player) {
			quickslot[selected_item, QSlot.Barrel] = 0
		}
	}	
