function grab_nade_animation() {
	if nadeIndex == noone exit;
	...
	stab = lerp(stab, 0, lerp_str)

	var flip = (mouse_x > x) ? 1 : -1;

	if stab > 20 {
	draw_sprite_ext(spr_player_arm_throw, 0, gun_x, gun_y, 1, flip, dir, c_white, 1);
	smooth_recoil_x -= lengthdir_x(stab, mouse_dir)
	smooth_recoil_y -= lengthdir_y(stab, mouse_dir)
	} else {	
	draw_sprite_ext(nade[nadeIndex, MELEE.ARMED_SPR], 0, gun_x, gun_y, 1, flip, dir, c_white, 1);
}}